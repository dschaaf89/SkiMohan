...
import { Billboard, Program } from "@prisma/client"
...
export const ProgramForm: React.FC<ProgramFormProps> = ({
  ...
  const onSubmit = async (data: ProgramFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.seasonId}/programs/${params.ProgramId}`, data);
      } else {
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.seasonId}/programs/${params.ProgramId}`);
      router.refresh();
      router.push(`/${params.seasonId}/programs`);
      toast.success('Program deleted.');
    } catch (error: any) {
      toast.error('Make sure you removed all products using this Program first.');
    } finally {
      setLoading(false);
      setOpen(false);
    }
